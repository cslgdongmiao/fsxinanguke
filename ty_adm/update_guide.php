...
else if($dopost=='getlist')
{
    ...
    else
    {
        ...
        $allFileList .= "文件临时存放目录：../0tydt/<input type='text' name='tmpdir' style='width:200px' value='$tmpdir' /><br />\r\n";
...
else if($dopost=='down')
{
    ...
    else
    {
        ...
        ShowMsg("完成所有远程文件获取操作：<a href='update_guide.php?dopost=apply'>&lt;&lt;点击此开始直接升级&gt;&gt;</a><br />你也可以直接使用[../0tydt/{$tmpdir}]目录的文件手动升级。","javascript:;");
...
else if($dopost=='apply')
{
    ...
    else
    {
        ...
        if($badcp > 0)
        {
            $badmsg = "，其中失败 {$badcp} 个文件，<br />请从临时目录[../0tydt/{$tmpdir}]中取出这几个文件手动升级。";
        }

...
class DedeSqli
{
	...
	{
		$RecordLogFile = dirname(__FILE__).'/../0tydt/mysqli_record_log.inc';
		$url = $this->GetCurUrl();
		$savemsg = <<<EOT
...
EOT;
    ...
    {
        $errorTrackFile = dirname(__FILE__).'/../0tydt/mysqli_error_trace.inc';
        if( file_exists(dirname(__FILE__).'/../0tydt/mysqli_error_trace.php') )
        {
            @unlink(dirname(__FILE__).'/../0tydt/mysqli_error_trace.php');
        }
		if($this->showError)
		{
			$emsg = '';
...
if (!function_exists('CheckSql'))
{
    function CheckSql($db_string,$querytype='select')
    {
        ...
        $old_pos = 0;
        $pos = -1;
        $log_file = DEDEINC.'/../0tydt/'.md5($cfg_cookie_encode).'_safe.txt';
        $userIP = GetIP();
        $getUrl = GetCurUrl();

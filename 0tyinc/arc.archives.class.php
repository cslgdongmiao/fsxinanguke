...
class Archives
{
    ...
    function ParAddTable()
    {
        ...
        if (isset($this->Fields['voteid']) && !empty($this->Fields['voteid']))
        {
            ...
            $this->Fields['vote'] = "<script language='javascript' src='{$GLOBALS['cfg_cmspath']}/0tydt/vote/vote_{$voteid}.js'></script>";
            if ($GLOBALS['cfg_multi_site'] == 'Y')
            {
                $this->Fields['vote'] = "<script language='javascript' src='{$GLOBALS['cfg_mainsite']}/0tydt/vote/vote_{$voteid}.js'></script>";
            }
        }

...
if($action == 'post')
{
    ...
    elseif($do == 2)
    {
		...
		$mailtitle = "【手机-上海强直预约】速回电话-{$phone}";
		$mailbody = "姓名:{$name}，回电号码:{$phone}，年龄:{$age}，备注:{$beizhu}";
		$headers = $cfg_adminemail;
		$mailtype = 'TXT';
		require_once(DEDEINC.'/mail.class.php');
		$smtp = new smtp($cfg_smtp_server,$cfg_smtp_port,true,$cfg_smtp_usermail,$cfg_smtp_password);
		$smtp->debug = false;
		$smtp->sendmail("1619585687@qq.com","上海强直",$cfg_smtp_usermail, $mailtitle, $mailbody, $mailtype);
        if($dsql->ExecuteNoneQuery($query))
        {
            ...
            {
                $goto = 'http://m.shzzyy2013.com/js/ok.html';
                $bkmsg = '发布成功，请等待管理员处理...';
            }
            showmsg($bkmsg, $goto);
        }

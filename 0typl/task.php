 ...
 * ·JS触发方式：在所有文档页面中用JS调用/0typl/task.php?client=js（必须禁用计划任务的密码，系统配置参数->其它选项）；
 * ·自行定制客户端：直接访问“http://网址/0typl/task.php?clientpwd=管理密码”，会返回其中一个可执行任务的网址（没有可用任务则返回串：notask)，然后客户端运行这个网址即可。 
 *
